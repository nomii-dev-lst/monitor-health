...
export const monitors = pgTable('monitors', {
  ...
  validationRules: jsonb('validation_rules').notNull().default({ statusCode: 200 }),
  ...
  return {
    ...
    enabledNextCheckIdx: index('enabled_next_check_idx').on(table.enabled, table.nextCheckTime),
...
export const checkResults = pgTable('check_results', {
  ...
  return {
    ...
    monitorIdCheckedAtIdx: index('monitor_id_checked_at_idx').on(table.monitorId, table.checkedAt),
    statusCheckedAtIdx: index('status_checked_at_idx').on(table.status, table.checkedAt),
...
export const alerts = pgTable('alerts', {
  ...
  return {
    ...
    monitorIdSentAtIdx: index('monitor_id_sent_at_idx').on(table.monitorId, table.sentAt),
