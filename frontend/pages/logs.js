...
export default function LogHistory() {
  ...
  const { logs, logsStats, logsPagination, loadLogs, loadLogsStats, setLogs, setLogsStats, setLogsPagination, isRunningAll, runAllProgress } = useDashboard();
  ...
  }, [authLoading, isAuthenticated, logsPagination.offset, loadLogs, loadLogsStats, logsPagination.limit]);
  ...
  useEffect(() => {
    if (!authLoading && isAuthenticated) {
      ...
      const eventSource = new EventSource(`${backendUrl}/api/sse/events?token=${encodeURIComponent(token)}`);
      ...
      eventSource.addEventListener('log', (event) => {
        ...
        if (logsPagination.offset === 0) {
          setLogs(prevLogs => {
            ...
            const updated = [newLog, ...prevLogs].slice(0, logsPagination.limit);
            return updated;
          });
  ...
  }, [authLoading, isAuthenticated, logsPagination.offset, logsPagination.limit, setLogs, setLogsStats]);
      ...
      <div className="min-h-screen space-y-6">
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center space-x-3">
            ...
            {isRunningAll && (
              <div className="flex items-center space-x-2 px-3 py-2 bg-primary-50 border border-primary-200 rounded-md">
        ...
        {logsStats && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="text-sm font-medium text-gray-500">Total Checks (24h)</div>
              <div className="mt-2 text-3xl font-bold text-gray-900">{logsStats.totalChecks}</div>
            </div>
            
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="text-sm font-medium text-gray-500">Successful</div>
              <div className="mt-2 text-3xl font-bold text-green-600">{logsStats.successfulChecks}</div>
            </div>
            
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="text-sm font-medium text-gray-500">Failed</div>
              <div className="mt-2 text-3xl font-bold text-red-600">{logsStats.failedChecks}</div>
            </div>
            
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="text-sm font-medium text-gray-500">Success Rate</div>
              <div className="mt-2 text-3xl font-bold text-primary-600">{logsStats.successRate}%</div>
            </div>
          </div>
        )}
                  ...
                  filteredLogs.map((log) => (
                    <tr 
                      ...
                      className={`hover:bg-gray-50 transition-colors duration-300 ${
                        newLogIds.has(log.id) ? 'bg-primary-50 animate-pulse' : ''
                      }`}
                    >
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <div className="font-medium">{log.monitorName || 'Unknown'}</div>
          ...
          {filteredLogs.length > 0 && (
            <div className="bg-gray-50 px-6 py-4 flex items-center justify-between border-t border-gray-200">
              <div className="text-sm text-gray-700">
                Showing {logsPagination.offset + 1} to {Math.min(logsPagination.offset + logsPagination.limit, logsPagination.total)} of {logsPagination.total} results
              </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{selectedLog.monitorName}</p>
                  <p className="text-xs text-gray-500">{selectedLog.monitorUrl}</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Status</label>
                    <p className="mt-1"><StatusBadge status={selectedLog.status} size="md" /></p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">HTTP Status</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedLog.httpStatus || 'N/A'}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedLog.latency ? `${selectedLog.latency}ms` : 'N/A'}</p>
                  </div>
                  <div>
                ...
                {selectedLog.validationErrors && selectedLog.validationErrors.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Validation Errors</label>
                ...
                {selectedLog.responseData && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Response Data</label>
